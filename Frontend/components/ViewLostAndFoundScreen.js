...
const ViewLostAndFoundScreen = () => {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://192.168.0.188:5000/api/lost-found');
  ...
  const handleReplySubmit = async (itemId) => {
    ...
    try {
      const response = await fetch(`http://192.168.0.188:5000/api/lost-found/reply`, {
        method: 'POST',
              ...
              {item.images && item.images.length > 0 ? (
                <ScrollView horizontal style={styles.imageContainer}>
                  {item.images.map((img, index) => (
                    <Image 
                      key={index} 
                      source={{ uri: `http://192.168.0.188:5000${img}` }} 
                      style={styles.image} 
                    />
                  ))}
                </ScrollView>
